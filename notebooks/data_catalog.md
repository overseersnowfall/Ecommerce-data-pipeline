...
## Overview
The Gold Layer is the business-level data representation, structured to support analytical and reporting use cases. It consists of **dimension tables** and **fact tables** for specific business metrics. All tables reside in the `gold_layer` schema and are loaded via the `gold_layer.load_gold_full()` stored procedure, sourcing data from the `silver_layer`.
